...
public class TableSubModuleView extends SubModuleView {
	...
	protected void basicCreatePartControl(final Composite parent) {
		...
		parent.setLayout(new DpiGridLayout(1, false));
	...
	private Group createTableGroup(final Composite parent) {
		...
		DpiGridLayoutFactory.fillDefaults().margins(20, 20).numColumns(1).applyTo(group);

		final Composite tableComposite = new Composite(group, SWT.NONE);
		GridDataFactory.fillDefaults().grab(true, true).applyTo(tableComposite);

		final Table table = UIControlsFactory.createTable(tableComposite, SWT.MULTI | SWT.BORDER | SWT.FULL_SELECTION, "table"); //$NON-NLS-1$
		...
		final DpiTableColumnLayout layout = new DpiTableColumnLayout();
		layout.setColumnData(columnState, new ColumnPixelData(30, false));
		layout.setColumnData(columnWord, new ColumnWeightData(25, 30));
	...
	private Composite createButtonComposite(final Group group) {
		...
		DpiGridLayoutFactory.fillDefaults().numColumns(4).equalWidth(false).applyTo(buttonComposite);

		final Button buttonPrintSelection = UIControlsFactory.createButtonCheck(buttonComposite, "", "buttonPrintSelection"); //$NON-NLS-1$ //$NON-NLS-2$
		GridDataFactory.fillDefaults().grab(true, false).align(SWT.END, SWT.CENTER).hint(120, SWT.DEFAULT).applyTo(buttonPrintSelection);
