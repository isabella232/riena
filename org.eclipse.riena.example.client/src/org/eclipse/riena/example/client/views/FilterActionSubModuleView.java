 ...
 */
public class FilterActionSubModuleView extends SubModuleView<FilterNavigationSubModuleController> {
	...
	private Group createFiltersGroup(Composite parent) {
		...
		Group group = UIControlsFactory.createGroup(parent, "UI-Filters (MenuItem)"); //$NON-NLS-1$
		...
		Label label1 = UIControlsFactory.createLabel(group, "Item ID"); //$NON-NLS-1$
		GridDataFactory.fillDefaults().grab(false, false).span(1, 1).applyTo(label1);
		Text itemId = UIControlsFactory.createText(group);
		addUIControl(itemId, "itemId"); //$NON-NLS-1$
		GridDataFactory.fillDefaults().grab(true, false).span(1, 1).applyTo(itemId);
