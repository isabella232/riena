...
public class CloseModuleTest extends RienaTestCase {
	...
	protected void setUp() throws Exception {
		...
		a2Module1 = EasyMock.createMock(IModuleNode.class);
		a2Module2 = EasyMock.createMock(IModuleNode.class);
	}
	...
	public void testFindModule() {
		...
		EasyMock.expect(a2ModuleGroup1.getChildren()).andReturn(Arrays.asList(a2Module1, a2Module2));
		EasyMock.expect(a2Module1.isActivated()).andReturn(false);
		EasyMock.expect(a2Module2.isActivated()).andReturn(true);
		enableReplay();

		assertSame(a2Module2, handler.findModule(appNode));
	}
	...
	public void testFindModule_NoSubApps() {
		...
		assertNull(handler.findModule(appNode));
	}
	...
	public void testFindModule_NoModules() {
		...
		EasyMock.expect(subApp2.getChildren()).andReturn(Arrays.asList(a2ModuleGroup1));
		EasyMock.expect(a2ModuleGroup1.isActivated()).andReturn(true);
		EasyMock.expect(a2ModuleGroup1.getChildren()).andReturn(new ArrayList<IModuleNode>());
		enableReplay();
	...
	public void testFindModule_NoActiveModules() {
		...
		EasyMock.expect(a2ModuleGroup1.getChildren()).andReturn(Arrays.asList(a2Module1, a2Module2));
		EasyMock.expect(a2Module1.isActivated()).andReturn(false);
		EasyMock.expect(a2Module2.isActivated()).andReturn(false);
		enableReplay();
