...
/**
 * <p>
 * A proxy for a handler that has been defined in XML. This delays the class loading until the handler is really asked for information (besides the priority or
 * the command identifier). Asking a proxy for anything but the attributes defined publicly in this class will cause the proxy to instantiate the proxied
 * handler.
 * </p>
 * 
 * @since 3.0
...
public final class HandlerProxy extends AbstractHandlerWithState implements IElementUpdater {
	...
	/**
	 * The configuration element from which the handler can be created. This value will exist until the element is converted into a real class -- at which point
	 * this value will be set to <code>null</code>.
	 */
	private IConfigurationElement configurationElement;

	/**
	 * The <code>enabledWhen</code> expression for the handler. Only if this expression evaluates to <code>true</code> (or the value is <code>null</code>)
	 * should we consult the handler.
	 */
	private final Expression enabledWhenExpression;

	/**
	 * The real handler. This value is <code>null</code> until the proxy is forced to load the real handler. At this point, the configuration element is
	 * converted, nulled out, and this handler gains a reference.
	 */
	private IHandler handler = null;

	/**
	 * The name of the configuration element attribute which contains the information necessary to instantiate the real handler.
	 */
	...
	/**
	 * The evaluation service to use when evaluating <code>enabledWhenExpression</code>. This value may be <code>null</code> only if the
	...
	/**
	 * Constructs a new instance of <code>HandlerProxy</code> with all the information it needs to try to avoid loading until it is needed.
	 * 
	 * @param commandId
	 *            the id for this handler
	 * @param configurationElement
	 *            The configuration element from which the real class can be loaded at run-time; must not be <code>null</code>.
	 * @param handlerAttributeName
	 *            The name of the attibute or element containing the handler executable extension; must not be <code>null</code>.
	 */
	public HandlerProxy(final String commandId, final IConfigurationElement configurationElement, final String handlerAttributeName) {
	 ...
	 *            The configuration element from which the real class can be loaded at run-time; must not be <code>null</code>.
	 * @param handlerAttributeName
	 *            The name of the attribute or element containing the handler executable extension; must not be <code>null</code>.
	 * @param enabledWhenExpression
	 *            The name of the element containing the enabledWhen expression. This should be a child of the <code>configurationElement</code>. If this value
	 *            is <code>null</code>, then there is no enablement expression (i.e., enablement will be delegated to the handler when possible).
	 * @param evaluationService
	 *            The evaluation service to manage enabledWhen expressions trying to evaluate the <code>enabledWhenExpression</code>. This value may be
	 *            <code>null</code> only if the <code>enabledWhenExpression</code> is <code>null</code>.
	 */
	public HandlerProxy(final String commandId, final IConfigurationElement configurationElement, final String handlerAttributeName,
			final Expression enabledWhenExpression, final IEvaluationService evaluationService) {
		...
		if ((enabledWhenExpression != null) && (evaluationService == null)) {
			throw new NullPointerException("We must have a handler service and evaluation service to support the enabledWhen expression"); //$NON-NLS-1$
		}
	...
	public static void updateStaleCEs(final IConfigurationElement[] replacements) {
		for (final IConfigurationElement replacement : replacements) {
			final HandlerProxy proxy = (HandlerProxy) CEToProxyMap.get(replacement);
			if (proxy != null) {
	...
	private void registerEnablement() {
		enablementRef = evaluationService.addEvaluationListener(enabledWhenExpression, getEnablementListener(), PROP_ENABLED);
	}
	...
	void setProxyEnabled(final boolean enabled) {
	...
	public final Object execute(final ExecutionEvent event) throws ExecutionException {
		if (loadHandler()) {
			if (!isEnabled()) {
				MessageDialog.openInformation(Util.getShellToParentOn(), WorkbenchMessages.Information, WorkbenchMessages.PluginAction_disabledMessage);
				return null;
			}
	...
	/**
	 * Loads the handler, if possible. If the handler is loaded, then the member variables are updated accordingly.
	 * 
	 * @return <code>true</code> if the handler is now non-null; <code>false</code> otherwise.
	 */
	...
	public final String toString() {
		if (handler == null) {
			if (configurationElement != null) {
				final String configurationElementAttribute = getConfigurationElementAttribute();
	...
	/**
	 * Retrives the ConfigurationElement attribute according to the <code>handlerAttributeName</code>.
	 * 
	 * @return the handlerAttributeName value, may be <code>null</code>.
	 */
	private String getConfigurationElementAttribute() {
		final String attribute = configurationElement.getAttribute(handlerAttributeName);
		if (attribute == null) {
			final IConfigurationElement[] children = configurationElement.getChildren(handlerAttributeName);
			for (final IConfigurationElement element : children) {
				final String childAttribute = element.getAttribute(IWorkbenchRegistryConstants.ATT_CLASS);
	...
	public void updateElement(final UIElement element, final Map parameters) {
		if (checkedState != null) {
			final Boolean value = (Boolean) checkedState.getValue();
			element.setChecked(value.booleanValue());
		} else if (radioState != null) {
			final String value = (String) radioState.getValue();
			final Object parameter = parameters.get(RadioState.PARAMETER_ID);
