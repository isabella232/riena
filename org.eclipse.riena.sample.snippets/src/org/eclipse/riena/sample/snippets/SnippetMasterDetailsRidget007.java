...
/**
 * Demonstrates how to use {@link IMasterDetailsDelegate#isRemovable(Object)} to
 * disallow removal of any 'Jackson' family member.
 */
public final class SnippetMasterDetailsRidget007 {
	...
	private static final class PersonDelegate extends AbstractMasterDetailsDelegate {
		...
		public String isRemovable(final Object item) {
			...
			if ("jackson".equalsIgnoreCase(person.getLastname())) { //$NON-NLS-1$
				result = String.format("Cannot remove '%s' family members.", person.getLastname()); //$NON-NLS-1$
			}
			return result;
		}
	}
