...
/**
 * Demonstrates a master details ridget that applies changes instantly.
 */
public final class ToggleInMasterDetailsBug2 {

	private ToggleInMasterDetailsBug2() {
		// "utility class"
	}
	...
	private static final class PersonMasterDetails extends MasterDetailsComposite {
		...
		protected void createDetails(final Composite parent) {
			...
			UIControlsFactory.createLabel(parent, "Has Dog:"); //$NON-NLS-1$
			final Button btnHasDog = UIControlsFactory.createButtonCheck(parent);
			btnHasDog.setText("wrong value!!");
			hFill.applyTo(btnHasDog);
			addUIControl(btnHasDog, "btnHasDog"); //$NON-NLS-1$

			final Label lblResult = UIControlsFactory.createLabel(parent, "<value>"); //$NON-NLS-1$
			hFill.span(2, 1);
			hFill.applyTo(lblResult);
			addUIControl(lblResult, "lblResult");
		}
	}
	...
	private static final class PersonDelegate extends AbstractMasterDetailsDelegate {
		...
		private ILabelRidget lblResult;
		...
		public void configureRidgets(final IRidgetContainer container) {
			...
			final IToggleButtonRidget hasDog = container.getRidget(IToggleButtonRidget.class, "btnHasDog");
			hasDog.bindToModel(workingCopy, "hasDog");

			lblResult = container.getRidget(ILabelRidget.class, "lblResult");
		}
		...
		public Person copyBean(final Object source, final Object target) {
			...
			to.setHasDog(from.isHasDog());
			if (lblResult != null) {
				lblResult.setText("Has Dog (from): " + from.isHasDog());
			}
			return to;
		}
		...
		public boolean isChanged(final Object source, final Object target) {
			...
			final boolean equal = p1.getFirstname().equals(p2.getFirstname())
					&& p1.getLastname().equals(p2.getLastname()) && p1.isHasDog() == p2.isHasDog();
			return !equal;
		}
		 ...
		 * 
		 * @see
		 * org.eclipse.riena.ui.ridgets.AbstractMasterDetailsDelegate#updateDetails
	...
	public static void main(final String[] args) {
		...
		shell.setText(ToggleInMasterDetailsBug2.class.getSimpleName());
