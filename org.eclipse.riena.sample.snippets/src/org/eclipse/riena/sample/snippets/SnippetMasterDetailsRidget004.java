...
public final class SnippetMasterDetailsRidget004 {
	...
	private static final class PersonDelegate extends AbstractMasterDetailsDelegate {
		...
		public void configureRidgets(final IRidgetContainer container) {
			final ITextRidget txtLast = container.getRidget(ITextRidget.class, "txtLast"); //$NON-NLS-1$
			...
			final ITextRidget txtFirst = container.getRidget(ITextRidget.class, "txtFirst"); //$NON-NLS-1$
	...
	public static void main(final String[] args) {
		...
		final Button btnSuggest = UIControlsFactory.createButton(details, "&Suggest"); //$NON-NLS-1$
		...
		final IActionRidget btnRidget = (IActionRidget) SwtRidgetFactory.createRidget(btnSuggest);
		btnRidget.addListener(new IActionListener() {
			private int count;

			public void callback() {
				if (ridget.canSuggest()) {
					final Person person = new Person("Max " + ++count, "Mustermann"); //$NON-NLS-1$ //$NON-NLS-2$
